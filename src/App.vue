...
export default class App extends Vue {
  ...
  public get locations(): TLocation[] {
    return this.weatherForecastList.map(({ location }): TLocation => location);
  }
  ...
  public getWeatherByCityName(name: string): void {
    WeatherService.getWeatherByCityName(name)
      .then((weatherForecast: TWeatherForecast) => {
        if (
          !this.locations.find(({ id }) => id === weatherForecast.location.id)
        ) {
  ...
  public removeLocationById(id: number): void {
    this.weatherForecastList = this.weatherForecastList.filter(
      (weatherForecast) => weatherForecast.location.id !== id
    );
    LocationService.saveLocalLocations(this.locations);
  }
