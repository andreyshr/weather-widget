<template>
  <div class="settings">
    ...
    <container @drop="$emit('change-location-order', $event)" drag-handle-selector=".drag-handle">
      <draggable v-for="location in locations" :key="location.id">
        <settings-location :location="location" @remove-location="$emit('remove-location', $event)"></settings-location>
      </draggable>
    </container>
...
@Component({
  components: {IconButton, SettingForm, SettingsLocation, Container, Draggable}
})
