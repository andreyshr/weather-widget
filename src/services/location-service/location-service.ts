...
export class LocationService {
    ...
    public static getLocations(): Promise<TLocation[]> {
        ...
        if (Array.isArray(locations) && locations.length) return Promise.resolve(locations);

        const createEmptyLocation = (coordinates: TCoordinates): TLocation[] => [{coordinates, id: null, country: null, city: null}];

        return GeolocationModule
            .getLocation()
            .then(createEmptyLocation);
    }

    public static getLocalLocations(): Nullable<TLocation[]> {
        const locations = locationsStorage.getLocations();

        const isLocation = (location: TLocation): boolean => !!(location.id || location.coordinates);
